...
class LanguageModel:
  ...
  def train(self, tokens: list, verbose: bool = False) -> None:
    ...
    for i in range(len(tokens)):
      if tokenCount[tokens[i]] == 1:
        tokens[i] = UNK      
      self.vocab.add(tokens[i])
      self.valueN[tokens[i]] += 1

    for each_ngram in create_ngrams(tokens, self.n_gram):
        self.n_gramCount[each_ngram] += 1 
  ...
  def score(self, sentence_tokens: list) -> float:
    ...
    # STUDENTS IMPLEMENT
    sen_prob = 1    
    for each_ngram in create_ngrams(sentence_tokens, self.n_gram):
      if self.n_gram > 1:
        if each_ngram not in self.n_gramCount: 
          each_ngram = tuple(UNK if each_ngram[i] not in self.vocab else each_ngram[i] for i in range(len(each_ngram)))
        laplace_prob =  (self.n_gramCount[each_ngram] + 1) / (self.valueN[each_ngram[-2]] + len(self.vocab))
      else:
        if each_ngram not in self.n_gramCount: count_ngram = self.n_gramCount[(UNK,)]
        else: count_ngram = self.n_gramCount[each_ngram]
        laplace_prob =  ( count_ngram + 1) / (sum(self.valueN.values()) + len(self.vocab))
      sen_prob *= laplace_prob
    return sen_prob
  ...
  def generate_sentence(self) -> list:
    ...
    while sentence[-1] != SENTENCE_END:
      ...
      for each_ngramCount in self.n_gramCount:        
        if self.n_gram > 1 and each_ngramCount[-2] == sentence[-1]:
          if each_ngramCount[-1] != SENTENCE_BEGIN:
            possible_next_tokens.append((each_ngramCount[-1],self.n_gramCount[each_ngramCount]/self.valueN[each_ngramCount[-2]])) 
        elif self.n_gram == 1 and each_ngramCount[0] != SENTENCE_BEGIN:
          possible_next_tokens.append((each_ngramCount[0],self.n_gramCount[each_ngramCount]))
      possibleWords = [i[0] for i in possible_next_tokens]
      wordProb =  [i[1] for i in possible_next_tokens]
      sumWords = sum(wordProb)
      for j in range(len(wordProb)):
        wordProb[j] /= sumWords      
      next_token = np.random.choice(possibleWords, p=wordProb)
  ...
  def perplexity(self, sequence: list) -> float:
    ...
    """
    # 6120 IMPLEMENTS
    prob = self.score(sequence)
    perplexity = (1/prob) ** (1 / len(sequence))
